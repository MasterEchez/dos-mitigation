...
const recordMediaPath = "./puppeteer/record-media.js";
...
(async () => {
    ...
    try {
        ...
        let content = `const recordTime = ${windowLengthNum} * 3 * 1000 + 100;\n`;
            // + `const expStartDateTime = new Date("${expStartTime}");\n`;
        content += await fs.readFile(recordMediaPath, 'utf8');
        // console.log(content);
        while (new Date() < expStartDateTime) {
            await new Promise((resolve) => setTimeout(resolve, 20)); // Check every 20 ms
        }
        const recordMediaScript = await page.addScriptTag({content, id: scriptId});
        ...
        const [blob, finishedRecording, recorderstate1, recorderstate2, recorderstate3] = await page.evaluate(() => {
            // const recordMediaScript = document.getElementById(scriptId);
            return [blob, finishedRecording, recorderstate1, recorderstate2, recorderstate3];
        });
        console.log(finishedRecording);
        console.log(blob.type);
        console.log(recorderstate1);
        console.log(recorderstate2);
        console.log(recorderstate3);
